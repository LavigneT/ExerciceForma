...
class TestForms {
	...
	void testSquare() {
		...
		try {
			...
			//assertEquals(test1.getHeight(), 10);
			assertEquals(test1.getDiagonal()[0].getX(), 10);
			assertEquals(test1.getDiagonal()[0].getY(), 10);
			assertEquals(test1.getDiagonal()[1].getX(), -10);
			assertEquals(test1.getDiagonal()[1].getY(), 10);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
